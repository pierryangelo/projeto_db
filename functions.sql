...
BEGIN
    ...
    ELSE
        UPDATE PESSOA SET NOME_PESSOA = NOME, DT_NASC_PESSOA = DT_NASC, EMAIL_PESSOA = EMAIL, CPF = CPF_N WHERE COD_PESSOA = COD_PES;
        UPDATE FUNCIONARIO SET SALARIO = SALARIO_N WHERE COD_FUNCIONARIO = COD_FUNC;
        RETURN format('O funcionário (%1$s) foi atualizado!', COD_FUNC);
    END IF;
END;
$$ LANGUAGE 'plpgsql';

SELECT EXTRACT(YEAR FROM AGE(CURRENT_DATE, DT_NASC_PESSOA)) FROM PESSOA NATURAL JOIN FUNCIONARIO WHERE CPF = '222.222.222-22';
