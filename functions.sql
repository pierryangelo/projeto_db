...
AS $$
BEGIN
    RETURN OBTER_SALARIO_FUNCIONARIO(CPF, EXTRACT(MONTH FROM CURRENT_DATE)::INT, EXTRACT(YEAR FROM CURRENT_DATE)::INT);
END;
$$ LANGUAGE 'plpgsql';
...
AS $$
BEGIN
    RETURN OBTER_SALARIO_FUNCIONARIO(CPF, MES, EXTRACT(YEAR FROM CURRENT_DATE)::INT);
END;
$$ LANGUAGE 'plpgsql';
...
CREATE OR REPLACE FUNCTION CONTABILIZAR_PRODUTO(COD_ITEM INT, QUANT INT)
RETURNS void
AS $$
DECLARE
    QUANT_ATUAL INT;
BEGIN
    SELECT QUANT_ESTOQUE INTO QUANT_ATUAL FROM PRODUTO WHERE COD_PRODUTO = COD_ITEM;
    IF QUANT_ATUAL - QUANT < 0 THEN
        UPDATE PRODUTO SET QUANT_ESTOQUE = 0 WHERE COD_PRODUTO = COD_ITEM;
    ELSE
        UPDATE PRODUTO SET QUANT_ESTOQUE = QUANT_ESTOQUE - QUANT WHERE COD_PRODUTO = COD_ITEM;
    END IF;
END;
$$ LANGUAGE 'plpgsql';

CREATE OR REPLACE FUNCTION CONTABILIZAR_CARRINHO(COD_CART INT)
RETURNS void
AS $$
DECLARE
    PRODUTOS TEXT;
    LINE RECORD;
BEGIN
    PRODUTOS := format('SELECT * FROM OBTER_CARRINHO(%1$s);', COD_CART);
    IF PRODUTOS IS NOT NULL THEN
        FOR LINE IN EXECUTE PRODUTOS LOOP
            PERFORM CONTABILIZAR_PRODUTO(LINE.COD_PRODUTO, LINE.QUANTIDADE);
        END LOOP;
    END IF;
END;
$$ LANGUAGE 'plpgsql';
...
BEGIN
    ...
    IF VALOR_RESTANTE = 0 THEN
        ...
        PERFORM CONTABILIZAR_CARRINHO(COD_CART);
...
DECLARE
    ...
    PAGO BOOLEAN;
BEGIN
   SELECT NOME_PESSOA INTO NOME_CLI FROM CLIENTE NATURAL JOIN PESSOA NATURAL JOIN CARRINHO WHERE COD_CARRINHO = COD_CART;
   SELECT NOME_PESSOA INTO NOME_FUNC FROM FUNCIONARIO NATURAL JOIN PESSOA NATURAL JOIN CARRINHO WHERE COD_CARRINHO = COD_CART;
   ...
   RETURN format('O carrinho do cliente %1$s, atendido pelo funcionário %2$s contém %3$s itens e tem valor total de R$ %4$s', NOME_CLI, NOME_FUNC, QUANT_TOTAL_PRODUTOS, PRECO_FINAL);
END;
$$ LANGUAGE 'plpgsql';
...
DECLARE
   COD_MARC INT;
BEGIN
    SELECT COD_MARCA INTO COD_MARC FROM MARCA WHERE NOME_MARCA ILIKE '%' || NOME || '%';
    ...
    ELSE
        UPDATE MARCA SET NOME_MARCA = NOVO_NOME, VISIVEL = TRUE WHERE COD_MARCA = COD_MARC;
...
DECLARE
   COD_CAT INT;
BEGIN
    SELECT COD_CATEGORIA INTO COD_CAT FROM CATEGORIA WHERE NOME_CATEGORIA ILIKE '%' || NOME || '%';
    ...
    ELSE
        UPDATE CATEGORIA SET NOME_CATEGORIA = NOVO_NOME, COD_MARCA = MARC, VISIVEL = TRUE WHERE COD_CATEGORIA = COD_CAT;
...
BEGIN
    ...
    UPDATE CATEGORIA SET VISIVEL = FALSE WHERE COD_CATEGORIA = COD_ITEM;
    RETURN format('A categoria (%1$s) foi deletada com sucesso!', COD_ITEM);
END;
$$ LANGUAGE 'plpgsql';
...
BEGIN
    ...
    UPDATE MARCA SET VISIVEL = FALSE WHERE COD_MARCA = COD_ITEM;
    RETURN format('A marca (%1$s) foi deletada com sucesso!', COD_ITEM);
END;
$$ LANGUAGE 'plpgsql';
