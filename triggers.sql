...
AS $$
BEGIN
    IF (SELECT EXTRACT(YEAR FROM AGE(CURRENT_DATE, DT_NASC_PESSOA)) FROM PESSOA NATURAL JOIN CLIENTE WHERE COD_PESSOA = NEW.COD_PESSOA) < 13 THEN
...
AS $$
BEGIN
    IF (SELECT EXTRACT(YEAR FROM AGE(CURRENT_DATE, DT_NASC_PESSOA)) FROM PESSOA NATURAL JOIN FUNCIONARIO WHERE COD_PESSOA = NEW.COD_PESSOA) < 16 THEN
